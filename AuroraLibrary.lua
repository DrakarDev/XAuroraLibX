...
Aurora.DelayPerSection = 0.03
Aurora.DelayPerElement = 0.01
...
local function createAcrylic(frame)
    ...
    local function updatePosition()
        ...
        local topRight = absPos + Vector2.new(absSize.X, 0)
...
local function elemFrame(parent)
    ...
    make("UICorner", { CornerRadius=sz(6), Parent=f })
    make("UIPadding", { PaddingTop=sz(6), PaddingBottom=sz(6), PaddingLeft=sz(6), PaddingRight=sz(6), Parent=f })
    make("UIListLayout", { SortOrder=Enum.SortOrder.LayoutOrder, Padding=sz(4), Parent=f })
...
local function _createMobileKeybind(title, onToggleCallback)
    if not (UserInputService.TouchEnabled or Aurora.MobileButtonOverride) then
        return nil, nil, nil
    end
    
    if not Aurora.MobileKeybindsGui then
        Aurora.MobileKeybindsGui = make("ScreenGui", { Name = "AuroraMobileKeybinds", ResetOnSpawn = false, DisplayOrder = 99995 })
        safeParent(Aurora.MobileKeybindsGui)
    end
    
    _mobileKeybindCount = _mobileKeybindCount + 1
    local thm = Aurora.Theme or Aurora.Themes.Dark
    local yOffset = s(150) + (_mobileKeybindCount - 1) * s(42)
    local defaultPos = UDim2.new(1, -s(56), 0, yOffset)
    ...
    local mobileStroke = make("UIStroke", { Color = thm.Border, Thickness = 1, Parent = mobileBtn })
    
    local shortText = title or "KB"
    if #shortText > 4 then
        shortText = shortText:sub(1, 4)
    end
    ...
    mobileBtn.MouseButton1Up:Connect(function()
        tw(mScale, { Scale = 1.0 }, 0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
    end)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if kbDrag and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
    ...
    mobileBtn.MouseButton1Click:Connect(function()
        if kbDragDist < 6 then
            pcall(onToggleCallback)
        end
    end)
    
    return mobileBtn, mobileStroke, mobileLbl
end
...
function Section:AddToggle(id, cfg)
    ...
    function obj:AddKeybind(kbId, kbCfg)
        ...
        local mobileBtn, mobileStroke, mobileLbl = _createMobileKeybind(kbCfg.Title or cfg.Title or title or "Toggle", function()
            set(not obj.Value)
        end)
        ...
        local function updateVisualState()
            ...
            if active then
                tw(kbBtn, {
                    BackgroundColor3 = Color3.fromRGB(15, 60, 30),
                    TextColor3 = Color3.fromRGB(38, 195, 95)
                }, 0.1)
                tw(kbStroke, {
                    Color = Color3.fromRGB(38, 195, 95)
                }, 0.1)
                if mobileBtn then
                    tw(mobileBtn, {
                        BackgroundColor3 = Color3.fromRGB(15, 60, 30)
                    ...
                    mobileLbl.TextColor3 = Color3.fromRGB(38, 195, 95)
                end
            else
                tw(kbBtn, {
                    BackgroundColor3 = currentThm.InputBG,
                    TextColor3 = currentThm.SubText
                }, 0.1)
                tw(kbStroke, {
                    Color = currentThm.Border
                }, 0.1)
                if mobileBtn then
                    tw(mobileBtn, {
                        BackgroundColor3 = currentThm.InputBG
...
function Section:AddAlert(cfg)
    ...
    local f=make(Aurora.FadeIn and "CanvasGroup" or "Frame",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundColor3=thm.Element,Parent=self.Container})
    make("UICorner",{CornerRadius=sz(7),Parent=f})
    make("UIPadding",{PaddingTop=sz(10),PaddingBottom=sz(10),PaddingLeft=sz(12),PaddingRight=sz(12),Parent=f})
    make("UIStroke",{Color=col,Thickness=1,Transparency=0.55,Parent=f})
    local bar=make("Frame",{Size=UDim2.new(0,s(3),1,-s(16)),Position=UDim2.new(0,-s(12),0,s(8)),BackgroundColor3=col,Parent=f})
    make("UICorner",{CornerRadius=sz(2),Parent=bar})
    make("UIListLayout",{SortOrder=Enum.SortOrder.LayoutOrder,Padding=sz(4),Parent=f})
    local header=make("Frame",{Size=UDim2.new(1,0,0,s(18)),BackgroundTransparency=1,Parent=f})
    local ico=make("ImageLabel",{Size=ss(16,16),Position=UDim2.new(0,0,0.5,0),AnchorPoint=Vector2.new(0,0.5),BackgroundTransparency=1,Parent=header})
    applyIcon(ico,imap[typ],col)
    make("TextLabel",{
        Size=UDim2.new(1,-s(22),1,0),Position=UDim2.new(0,s(22),0,0),
    ...
    if cfg.Content and cfg.Content ~= "" then
        make("TextLabel",{
            ...
            TextSize=fs(11),Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,TextWrapped=true,Parent=f,
        })
    end
...
function Section:AddParagraph(cfg)
    ...
    make("UIListLayout",{SortOrder=Enum.SortOrder.LayoutOrder,Padding=sz(4),Parent=txtF})
    local tx=0
    if cfg.Icon then
        local ico=make("ImageLabel",{Size=ss(16,16),BackgroundTransparency=1,Parent=txtF,LayoutOrder=-1})
        applyIcon(ico,cfg.Icon,thm.IconColor); tx=s(22)
    ...
    local contentLbl = make("TextLabel",{
        ...
        TextSize=fs(12),Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,TextWrapped=true,Parent=txtF,
    })
...
function Section:AddDropdown(id, cfg)
    ...
    local valBox=make("Frame",{Size=UDim2.new(0.5,0,1,0),Position=UDim2.new(0.5,0,0,0),BackgroundColor3=thm.InputBG,Parent=topF})
    make("UICorner",{CornerRadius=sz(5),Parent=valBox})
    ...
    local function populateOptions(vals)
        for _,val in ipairs(vals) do
            local optBtn=make("TextButton",{Size=UDim2.new(1,0,0,s(26)),BackgroundColor3=thm.InputBG,BackgroundTransparency=1,Text="",Parent=optionScroll})
    ...
    local function toggleDropdown()
        ...
        local elemCnt=math.min(5,#optionButtons)
        local targetH=dropExpanded and (s(24)+s(3)+elemCnt*s(28)+s(8)) or 0
...
function Section:AddInput(id, cfg)
    ...
    make("UICorner",{CornerRadius=sz(5),Parent=inputValBox})
...
function Section:AddKeybind(id, cfg)
    ...
    local mobileBtn, mobileStroke, mobileLbl = _createMobileKeybind(cfg.Title or "Keybind", function()
        active = not active
        updateVisualState()
        if Aurora.RefreshKeybindList then task.defer(Aurora.RefreshKeybindList) end
...
function Column:AddSection(title, cfg)
    ...
    local sectionObj = setmetatable({Container=cont, Frame=f}, Section)
...
function SubTab:AddSection(title, cfg) return Column.AddSection({Frame=self.ScrollContent},title,cfg) end
function SubTab:AddColumns()
    ...
    return setmetatable({Frame=l},Column), setmetatable({Frame=r},Column)
end

local Tab={}
Tab.__index=Tab
function Tab:AddSection(title, cfg) return Column.AddSection({Frame=self.ScrollContent},title,cfg) end
...
function Tab:AddSubTab(title)
    ...
    local subTabObj=setmetatable({Button=subBtn,Page=subPage,ScrollContent=subContent,Underline=underline},SubTab)
...
function Aurora:CreateWindow(cfg)
    ...
    Aurora.DelayPerTab = cfg.DelayPerTab or 0.08
    Aurora.DelayPerSection = cfg.DelayPerSection or 0.03
    Aurora.DelayPerElement = cfg.DelayPerElement or 0.01
    ...
    local function showClosePrompt()
        ...
        make("UICorner",{CornerRadius=sz(10),Parent=overlay})
        ...
        make("UICorner",{CornerRadius=sz(10),Parent=prompt})
    ...
    local win={Tabs={},Categories={},GUI=gui,MainFrame=main}
    ...
    local minimizeConn = UserInputService.InputBegan:Connect(function(input,processed)
    ...
    searchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        for _,cat in ipairs(win.Categories) do
            ...
            if query=="" then cat.Header.Visible=true; cat.Container.Visible=cat.Expanded
            else cat.Header.Visible=hasVis; cat.Container.Visible=hasVis end
        end
    end)
    ...
    function win:AddTab(tcfg, parentContainer)
        ...
        local t=setmetatable({Button=btn,Page=p,ScrollContent=c,DefaultScroll=defaultScroll,TextLabel=lbl,IconImg=ico,IconStr=tcfg.Icon,Indicator=indicator},Tab)
    ...
    local mobileGui
    if UserInputService.TouchEnabled or cfg.MobileButton == true then
        mobileGui = make("ScreenGui", { Name = "AuroraMobileToggleGui", ResetOnSpawn = false, DisplayOrder = 99999 })
        safeParent(mobileGui)
        
        local mbIcon = cfg.MobileButtonIcon or "solar/star-bold"
        local mbPos = cfg.MobileButtonPosition or UDim2.new(0, s(20), 0, s(120))
        ...
        local mobileBtn = make("TextButton", {
            ...
            Position = mbPos,
            BackgroundColor3 = thm.Background,
            BackgroundTransparency = self.Acrylic and 0.45 or 0.15,
            Text = "",
            ZIndex = 100,
            Parent = mobileGui
        })
        ...
        local mStroke = make("UIStroke", { Color = thm.Accent, Thickness = 1.5, Transparency = 0.3, Parent = mobileBtn })
        ...
        table.insert(self.ThemeObjs, { obj = mobileBtn, prop = "BackgroundColor3", key = "Background" })
        table.insert(self.ThemeObjs, { obj = innerGlow, prop = "BackgroundColor3", key = "Accent" })
        table.insert(self.ThemeObjs, { obj = mStroke, prop = "Color", key = "Accent" })
        ...
        local function wakeBtn()
            ...
            tw(mobileBtn, { BackgroundTransparency = (self.Acrylic and 0.45 or 0.15) }, 0.15)
            tw(innerGlow, { BackgroundTransparency = 0.8 }, 0.15)
            tw(mStroke, { Transparency = 0.3 }, 0.15)
        end

        local function snapToEdge()
            local screenWidth = mobileGui.AbsoluteSize.X
            if screenWidth == 0 then screenWidth = 800 end
            local btnX = mobileBtn.AbsolutePosition.X + mobileBtn.AbsoluteSize.X / 2
            local half = screenWidth / 2
            
            local targetX
            if btnX < half then
                targetX = s(20)
            else
                targetX = screenWidth - mobileBtn.AbsoluteSize.X - s(20)
            end
            
            tw(mobileBtn, { Position = UDim2.new(0, targetX, 0, mobileBtn.Position.Y.Offset) }, 0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
            
            task.delay(2.5, function()
                if not mDrag and tick() - lastInteractionTime >= 2.4 then
                    tw(mobileBtn, { BackgroundTransparency = 0.8 }, 0.3)
                    tw(innerGlow, { BackgroundTransparency = 0.95 }, 0.3)
                    tw(mStroke, { Transparency = 0.85 }, 0.3)
                end
            end)
        end
        ...
        task.spawn(function()
            while mobileBtn and mobileBtn.Parent do
                pcall(function()
                    if not mDrag then
                        local pulseMin = 0.15
                        local pulseThick = 2.0
                        if mobileBtn.BackgroundTransparency > 0.6 then
                            pulseMin = 0.65
                            pulseThick = 1.6
                        end
                        local t1 = TweenService:Create(mStroke, TweenInfo.new(1.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Transparency = pulseMin + 0.25, Thickness = pulseThick })
                        t1:Play()
                        t1.Completed:Wait()
                        local t2 = TweenService:Create(mStroke, TweenInfo.new(1.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Transparency = pulseMin, Thickness = 1.0 })
                        t2:Play()
                        t2.Completed:Wait()
                    else
                        task.wait(0.2)
    ...
    gui.Destroying:Connect(function()
        ...
        pcall(function()
            if minimizeConn then minimizeConn:Disconnect() end
        end)
    end)
